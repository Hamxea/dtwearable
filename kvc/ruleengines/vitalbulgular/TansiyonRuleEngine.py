    ...
    def execute(self, islem_dto, nabiz, yas, tansiyon_sistolik, tansiyon_diastolik, choosen_type, reference_table,
        ...
        if (120 <= tansiyon_sistolik <= 129) and (tansiyon_diastolik < 80):
            exception_list.append(
                RuleViolationException("Tansiyon Yükseldi. TANSIYON SİSTOLİK 120 ile 129 arasına çikti "
        ...
        if (tansiyon_sistolik >= 180) and (tansiyon_diastolik > 120):
            exception_list.append(
                RuleViolationException("Tansiyon Çök Yükseldi. HİPERTENSİF KRİZLER ", TansiyonEnum.HIPERTENSIF_KRIZLER,
                                       tansiyon_sistolik,
                                       reference_table, reference_id, prediction_id))

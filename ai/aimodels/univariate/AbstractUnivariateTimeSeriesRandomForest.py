...
class AbstractUnivariateTimeSeriesRandomForest(AbstractAIModel):
    """ Time series generation model using random_forest for univariate models """
    ...
    def train(self, dataset_parameters, hyperparameters):
        ...
        random_forest_model, mean_absolute_error_mean, mean_absolute_error_std = self.train_random_forest(X_train,
                                                                                                          y_train)

        score = self.test_random_forest(random_forest_model, X_test, y_test)

        return random_forest_model, {"Mean Absolute Error (mean)": mean_absolute_error_mean,
    ...
    def train_random_forest(self, X_train, y_train, kernel='rbf'):
        """Method that creates random_forest model using x_train and y_train
            Since random_forest scale is not insensitive, it is necessary to scale the data
        """
    ...
    def test_random_forest(self, random_forest_model, X_test, y_test):
        """ Method that calculates score using X_test and y_test on the created random_forest model """
        from sklearn.preprocessing import MinMaxScaler
        ...
        for i in range(len(X_test_scaled)):
            ...
            y_new = random_forest_model.predict(X_new)

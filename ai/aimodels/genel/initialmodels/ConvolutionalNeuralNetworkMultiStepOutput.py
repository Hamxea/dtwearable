...
class ConvolutionalNeuralNetworkMultiStepOutput(AbstractAIModel):
    ...
    def train(self, dataset_parameters, hyperparameters):
        ...
        X_train, X_test, y_train, y_test = self.split_dataset(df, dataset_parameters['test_ratio'],
                                                              hyperparameters['window_size_in'], hyperparameters['window_size_out'])
        cnn_model = self.train_cnn(X_train, y_train, hyperparameters['window_size_in'])
        graph = tf.get_default_graph()

        with graph.as_default():
            score, acc = self.test_cnn(cnn_model, X_test, y_test, hyperparameters['window_size_in'])

        return cnn_model, {"score": score, "accuracy": acc}

    def split_dataset(self, df, test_ratio, window_size_in, window_size_out):
        """ Method that divides dataset for train and test """

        X, y = self.split_sequences(df, window_size_in, window_size_out)

        return train_test_split(X, y, test_size=test_ratio, shuffle=False, stratify=None)

    def split_sequences(self, df, window_size_in, window_size_out):
        ...
        for i in range(len(sequences)):
            ...
            end_ix = i + window_size_in
            out_end_ix = end_ix + window_size_out
    ...
    def train_cnn(self, X_train, y_train, window_size_in):
        """ Method that creates cnn model using x_train and y_train """

        # choose a number of time steps
        #window_size_in, window_size_out = 3, 1
        # flatten output
        ...
        # define model
        model = Sequential()
        model.add(Conv1D(filters=64, kernel_size=2, activation='relu', input_shape=(window_size_in, n_features)))
    ...
    def test_cnn(self, cnn_model, X_test, y_test, window_size_in):
        ...
        #window_size_in = 3
        X_test = X_test.reshape(1, window_size_in, n_features)

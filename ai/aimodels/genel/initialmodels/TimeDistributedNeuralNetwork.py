...
class TimeDistributedNeuralNetwork(AbstractAIModel):
    ...
    def train(self, dataset_parameters, hyperparameters):
        ...
        time_distributed_model = self.train_time_distributed(X_train, y_train, dataset_parameters['window_size'])
        graph = tf.get_default_graph()

        with graph.as_default():
            score, acc = self.test_time_distributed(time_distributed_model, X_test, y_test, dataset_parameters['window_size'])
    ...
    def train_time_distributed(self, X_train, y_train, window_size):
        """ Method that creates time_distributed model using x_train and y_train """
        ...
        # define model
        model = Sequential()
        model.add(TimeDistributed(Dense(100), input_shape=(window_size, n_features)))
    ...
    def test_time_distributed(self, time_distributed_model, X_test, y_test, window_size):
        """ Method that calculates score using X_test and y_test on the created time_distributed model """
        # window_size = 3

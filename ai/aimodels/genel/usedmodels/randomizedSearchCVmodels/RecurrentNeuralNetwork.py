...
class RecurrentNeuralNetwork(AbstractAIModel):
    ...
    def train(self, dataset_parameters, hyperparameters):
        ...
        rnn_model = self.train_rnn(X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test,
        ...
        with graph.as_default():
            acc, loss, best_parameters, classifi_metrics_report, conf_matrix_evaluation = self.test_rnn(
                rnn_model, X_test, y_test, dataset_parameters['window_size'])

        return rnn_model, {"accuracy": acc, "classification metrics report": classifi_metrics_report,
    ...
    def create_model(self, init_mode, optimizer, units, activation,
        ...
        # define model
        model = Sequential()
        model.add(SimpleRNN(units, activation='relu', return_sequences=True, input_shape=(3, 10),
    ...
    def train_rnn(self, X_train, y_train, X_test, y_test, parameters, cross_validation, num_iteration, window_size):
    ...
    def test_rnn(self, rnn_model, X_test, y_test, window_size):
        ...
        yha_predict = rnn_model.predict(X_test)
        print(yha_predict)
        ...
        # (loss, acc)
        acc = rnn_model.score(X_test, y_test)

        # loss = log_loss(y_test, (np.reshape(yha_predict, (yha_predict.shape[0], 1))))
        # print("Loss:", loss)
        print("Test Accuracy:", acc)

        return rnn_model.best_score_, 0, rnn_model.best_params_, cl_report, cm

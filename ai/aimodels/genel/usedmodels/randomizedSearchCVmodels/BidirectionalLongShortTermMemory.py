...
class BidirectionalLongShortTermMemory(AbstractAIModel):
    ...
    def train(self, dataset_parameters, hyperparameters):
        ...
        bil_lstm_model = self.train_bil_lstm(X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test,
        ...
        with graph.as_default():
            acc, loss, best_parameters, classifi_metrics_report, conf_matrix_evaluation = self.test_bil_lstm(
                bil_lstm_model, X_test, y_test, dataset_parameters['window_size'])

        return bil_lstm_model, {"accuracy": acc, "classification metrics report": classifi_metrics_report,
    ...
    def create_model(self, init_mode, optimizer, units, activation, dropout_rate):
        ...
        model.add(Bidirectional(
            LSTM(units, activation=activation, return_sequences=True, input_shape=(3, 10), kernel_regularizer=l2(0.01),
    ...
    def train_bil_lstm(self, X_train, y_train, X_test, y_test, parameters, cross_validation, num_iteration,
                       window_size):
    ...
    def test_bil_lstm(self, bil_lstm_model, X_test, y_test, window_size):
        ...
        yha_predict = bil_lstm_model.predict(X_test)
        print(yha_predict)
        ...
        # (loss, acc)
        acc = bil_lstm_model.score(X_test, y_test)

        # loss = log_loss(y_test, (np.reshape(yha_predict, (yha_predict.shape[0], 1))))
        # print("Loss:", loss)
        print("Test Accuracy:", acc)

        return bil_lstm_model.best_score_, 0, bil_lstm_model.best_params_, cl_report, cm

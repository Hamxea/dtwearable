...
class GatedRecurrentNeuralNetwork(AbstractAIModel):
    ...
    def train(self, dataset_parameters, hyperparameters):
        ...
        gru_model = self.train_gru(X_train=X_train, y_train=y_train, X_test=X_test, y_test=y_test,
        ...
        with graph.as_default():
            acc, loss, best_parameters, classifi_metrics_report, conf_matrix_evaluation = self.test_gru(
                gru_model, X_test, y_test, dataset_parameters['window_size'])

        return gru_model, {"accuracy": acc, "classification metrics report": classifi_metrics_report,
    ...
    def split_sequences(self, df, window_size):
        ...
        train_df = df.iloc[:, -11:]  # Select last 9 columns (features and output) in a dataframe for training
    ...
    def create_model(self, init_mode, optimizer, units, activation,
        ...
        # define model
        model = Sequential()
        model.add(GRU(units, activation=activation, return_sequences=True, input_shape=(3, 10),
    ...
    def train_gru(self, X_train, y_train, X_test, y_test, parameters, cross_validation, num_iteration, window_size):
    ...
    def test_gru(self, gru_model, X_test, y_test, window_size):
        ...
        yha_predict = gru_model.predict(X_test)
        print(yha_predict)
        ...
        # (loss, acc)
        acc = gru_model.score(X_test, y_test)

        # loss = log_loss(y_test, (np.reshape(yha_predict, (yha_predict.shape[0], 1))))
        # print("Loss:", loss)
        print("Test Accuracy:", acc)

        return gru_model.best_score_, 0, gru_model.best_params_, cl_report, cm
